...
def get_max_rpm_version(package_name):
    """
    ...
    filenames = glob.glob("%s-[0-9]*.noarch.rpm" % package_name)
...
""" % ca_cert)
    ...
    command = [
        "/usr/bin/openssl", "req", "-passin", "pass:%s", "-text",
        "-config", cleanupAbsPath(configFile.filename),
        "-new", "-x509", "-days", d['--cert-expiration'],
        "-%s" % MD, "-key", cleanupAbsPath(ca_key), "-out", cleanupAbsPath(ca_cert)
    ]
...
def genServerKey(d, verbosity=0):
    ...
    command = ["/usr/bin/openssl", "genrsa", "-out", cleanupAbsPath(server_key), "4096"]
...
def genServerRpm(d, verbosity=0):
    ...
    hdr = get_max_rpm_version(server_rpm_name)
